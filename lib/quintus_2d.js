 ...
 Q.TileLayer = Q.Sprite.extend({
  ...
  Q.component('aiBounce', {
    added: function() {
      ...
      this.entity.on("bump.top",this,"goTop");
      this.entity.on("bump.bottom",this,"goBottom");
    },
    ...
    goLeft: function(col) {
      ...
      var ta = this.entity.player;
      var c = Math.sqrt(Math.pow(Math.abs(ta.p.x - this.entity.p.x),2)+Math.pow(Math.abs(ta.p.y - this.entity.p.y),2));
      //if (c <= 400) {
        var ax = 180 * (ta.p.x - this.entity.p.x)/c;
        var by = 180 * (ta.p.y - this.entity.p.y)/c;
        this.entity.p.vx = ax;
        this.entity.p.vy = by;
      /*}
      else{
        var tx = 3120;
        var ty = 1800;
        var c = Math.sqrt(Math.pow(Math.abs(tx - this.entity.p.x),2)+Math.pow(Math.abs(ty - this.entity.p.y),2));
        var ax = 20 * (tx - this.entity.p.x)/c;
        var by = 20 * (ty - this.entity.p.y)/c;
        this.entity.p.vx = ax;
        this.entity.p.vy = by;
      }*/
    },
    ...
    goBottom: function(col){
      ...
      /*}
        ...
        var ty = 1800;
        var c = Math.sqrt(Math.pow(Math.abs(tx - this.entity.p.x),2)+Math.pow(Math.abs(ty - this.entity.p.y),2));
        var ax = 30 * (tx - this.entity.p.x)/c;
        var by = 30 * (ty - this.entity.p.y)/c;
        this.entity.p.vx = ax;
        this.entity.p.vy = by;
      }*/
    },
